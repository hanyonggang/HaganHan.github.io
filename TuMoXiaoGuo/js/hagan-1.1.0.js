const hagan = {//根据设备判断touch或者click
    ...
    //设置移动端viewport所在meta标签样式  hagan.fnSetMeta(eMeta);  !IE8
    ...
    //动态获取1rem的大小 hagan.fnSetRem(oHtml);  !IE8
    ...
    //得到rem布局对应的像素尺寸
    fnGetPxSize(nPx, designWidth = 750) {
        nPx = parseInt(nPx);
        let nResult;
        if (this.fnTypeof(this.jInfo.rem) === "Function") {
            nResult = nPx;
        } else {
            nResult = 30 * nPx / designWidth * this.jInfo.rem;
        }
        return nResult;
    },
