...
//碰撞检测  如果this碰到obj则返回true，否则返回false;
...
//返回一个json:数组最小值与最小值下标 {"key":2,"value":21};
...
function fnMoveElasticity(obj, attr, iTarget, fn) {
    var iSpeed = 0;
    var style = parseFloat(obj.fnGetStyle(attr));//执行之处时的样式数值，用来存续小数
    clearInterval(obj.timer);
    obj.timer = setInterval(function () {
...
//拖拽运动      fnMoveDrag(obj,'bottom')      //如果没有Collision参数则只是普通的拖拽运动
...
//碰撞运动      fnMoveCollide(obj,10,10,'bottom')
